...
from lg3.lib.models.decode import XcodeYtimeDecoder
...
def build_transformer_input(x):
    bsz, tin, sin = x.shape
    x_feat = torch.permute(x, (0, 2, 1))  # (B, Sin, Tin)
    x_feat = x_feat.reshape(bsz * sin, tin).unsqueeze(-1)  # (B*Sin, Tin, 1)
    x_feat = torch.permute(x_feat, (1, 0, 2))  # (Tin, B*Sin, 1)
    return x_feat
...
def train_one_epoch(
    ...
    for i, data in enumerate(dataloader):
        ...
        y_pred_norm = model_decode(xcodes)  # (B*Sin, Tout)
        bsz, tin, sin = x.shape
        tout = y.shape[1]
...
def default_argument_parser():
    parser = argparse.ArgumentParser(description="LG3 Raw Forecaster")
