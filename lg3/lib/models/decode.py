...
"""
Helper models that convert codes to time.

* Transformer: takes a sequence of length N of D-dimensional codes (N, B, D) and
  predicts a N-length sequence in time (N, B, C) where C is the compression factor.
* MuStdModel: simple MLP that predicts mean and std of the future time series.
"""
...
class Transformer(nn.Module):
    ...
    def forward(self, codes: torch.Tensor, codes_mask: torch.Tensor = None) -> torch.Tensor:
        ...
        codes_output = self.linear_out(codes_output)
        return codes_output
