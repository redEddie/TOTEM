...
def train_one_epoch(
    ...
    for i, data in enumerate(dataloader):
        ...
        ymeanstd = ymeanstd.reshape((B, Sout, 2))
...
def train(args):
    ...
    params = get_params(args.data_type, args.data_path, args.batchsize)
...
def get_params(data_type, data_path, batchsize_override):
    if data_type == "lg3":
        dataroot = data_path
        sample = np.load(os.path.join(dataroot, "train_x_original.npy"))
        Sin = Sout = sample.shape[-1]
        batchsize = batchsize_override if batchsize_override > 0 else 64
    else:
