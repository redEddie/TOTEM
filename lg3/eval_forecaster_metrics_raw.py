...
def build_transformer_input(x):
    bsz, tin, sin = x.shape
    x_feat = torch.permute(x, (0, 2, 1))
    x_feat = x_feat.reshape(bsz * sin, tin).unsqueeze(-1)
    x_feat = torch.permute(x_feat, (1, 0, 2))
    return x_feat
...
def inference_batch(x, y, model_decode, revin, device):
    x = x.to(device)
    x_norm = revin(x, "norm")
    xcodes = build_transformer_input(x_norm)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate raw forecaster and export per-sequence metrics")
    ...
    parser.add_argument("--output_csv", type=str, default="lg3/results/forecast_metrics_raw.csv")
    parser.add_argument("--output_seq_mse_csv", type=str, default="lg3/results/forecast_seq_mse_raw.csv")
    parser.add_argument("--output_seq_mae_csv", type=str, default="lg3/results/forecast_seq_mae_raw.csv")
    parser.add_argument("--output_seq_mape_csv", type=str, default="lg3/results/forecast_seq_mape_raw.csv")
    parser.add_argument("--output_seq_mase_csv", type=str, default="lg3/results/forecast_seq_mase_raw.csv")
