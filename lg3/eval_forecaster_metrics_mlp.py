...
def inference_batch(
    ...
    bsz, _, sin = codeids_x.shape
    tout = y.shape[1]

    _ = model_mustd.revin_in(x, "norm")
    _ = model_mustd.revin_out(y.to(device), "norm")

    xcodes, _, _ = build_mlp_input(codeids_x, codebook, onehot=onehot)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate MLP forecaster and export per-sequence metrics")
    parser.add_argument("--data_dir", type=str, default="lg3/data/forecasting/Tin288_Tout96")
    ...
    parser.add_argument("--output_csv", type=str, default="lg3/results/forecast_metrics_mlp.csv")
    parser.add_argument("--output_seq_mse_csv", type=str, default="lg3/results/forecast_seq_mse_mlp.csv")
    parser.add_argument("--output_seq_mae_csv", type=str, default="lg3/results/forecast_seq_mae_mlp.csv")
    parser.add_argument("--output_seq_mape_csv", type=str, default="lg3/results/forecast_seq_mape_mlp.csv")
    parser.add_argument("--output_seq_mase_csv", type=str, default="lg3/results/forecast_seq_mase_mlp.csv")
    ...
    if not os.path.exists(feature_names_path):
        raise FileNotFoundError(
            ...
            "Re-run extract_forecasting_data to generate it."
        )
    ...
    with torch.no_grad():
        for i in range(0, len(x), args.batch_size):
            ...
            code_batch = torch.from_numpy(codeids_x[i : i + args.batch_size]).long()
